...
/* Motor output pin */
//#define MOTOR_P1A_DIR					TRISC5
//#define MOTO_OUT 						PC4
//#define MOTOR_P1B_DIR					TRISC4
//#define MOTO_P1B_OUT 					PC4
...
void Motor_Init(MOTOR_STATUS status)
{
	...
	else if(status == MOTOR_MODE_3)
	{
		PR2 = 125;//4ms
		CCPR1L = 47;//46.875->1.5ms
		CCP1CON = 0x8C | (0x00 << 4);
	}
...
void Motor_HandleStatusChangeEvt(MOTOR_STATUS status)
{
	if(!Motor_CheckRunLimit())
	{
		switch(status)
		{
			case MOTOR_MODE_STOP:
			{
				...
				{
					Motor_SetStatus(MOTOR_MODE_1);
					Motor_Start(MOTOR_MODE_1);
			...
			case MOTOR_MODE_1:
			{
				if(KEY_GetReleaseStatus())
				{
					...
					else
					{
						...
						Motor_SetStatus(MOTOR_MODE_2);
						Motor_Start(MOTOR_MODE_2);
			...
			case MOTOR_MODE_2:
			{
				if(KEY_GetReleaseStatus())
				{
					...
					else
					{
						...
						Motor_SetStatus(MOTOR_MODE_3);
						Motor_Start(MOTOR_MODE_3);
	...
	{
		/* LED Display */
		LEDTimer_DispRunLimitCmd(ENABLE);
